...
import { BarChart3, Crown, Dices, ListOrdered, Swords, Target, Trophy, Users } from "lucide-react";
...
/** 원정 fill 색상 보정: 홈팀 색상과 너무 비슷할 때 구분해 주기 위함 */
function ensureAwayFill(homeHex: string, awayHex: string, awayAccent?: string): string {
  ...
  return colorDist(home, lighter) >= colorDist(home, darker) ? rgbToHex(lighter[0], lighter[1], lighter[2]) : rgbToHex(darker[0], darker[1], darker[2]);
}
  ...
  return (
    <div className="sim1000-donut">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} role="img" aria-label="승률 도넛">
        <circle cx={cx} cy={cy} r={r} fill="none" stroke="var(--bp-line-soft, rgba(0,0,0,0.06))" strokeWidth={stroke} />
        {/* 홈 세그먼트 */}
        <circle
          cx={cx}
          cy={cy}
      ...
      <div className="sim1000-donut-center">
        ...
        <span
          ...
          style={decisive > 0 ? { color: homeDominant ? homeColor : awayNoteColor, fontWeight: 900 } : undefined}
        >
...
export function AiWinnerSimTab({ game, sim, homeLineup, awayLineup }: Props) {
  ...
  return (
    <section className="sim1000-detail-screen" style={{ padding: "8px 0" }}>
      {/* ── 평균 점수 헤더 ── */}
      <header className="sim1000-matchup" style={{ marginTop: 0 }}>
        ...
        <p className="sim1000-matchup-avg-note">평균 점수 (1000판 시뮬레이션 기준)</p>
      </header>
      ...
      <section className="sim1000-section">
        ...
        <DiffHistogram hist={sim.diff_hist ?? {}} n={safeN} homeColor={homeColor} awayColor={awayFill} />
      </section>
      ...
      {homeLineup || awayLineup ? (
        <section className="sim1000-section">
          ...
          <p className="sim1000-section-sub">시뮬레이션에 사용된 타순 및 선발 투수</p>
      ...
      {/* 라인업 소스 메타 */}
      <footer className="sim1000-footer" style={{ paddingBottom: "16px" }}>
